...
// DataEmail class inherits from Data class; it converts an e-mail in a text 
// file into training data.
...
class Data
{
 public:
  ...
  inline virtual int set_training_features(arma::mat training_features_arg) {
    training_features_ = arma::zeros(training_features_arg.n_rows,\
      training_features_arg.n_cols);
...
// Converts an e-mail in a text file into training data.  Sample usage:
// const std::string kEmailFileName = "email.txt";
// DataEmail data_email(kEmailFileName);
class DataEmail: public Data
{
 public:
  // Sets default values for training data.
  DataEmail() : Data() {}

  // Reads CSV file "email_file_name_arg".
  // Performs non-trivial initialization; will be defined in source file.
  explicit DataEmail(std::string email_file_name_arg);
 ...
 private:
  ...
  // Indices of stemmed words in a vocabulary file.
  std::vector<int> word_indices_;

  // Training features.
  arma::mat features_;

  DISALLOW_COPY_AND_ASSIGN(DataEmail);
};
